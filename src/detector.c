...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile, float thresh)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        for(k = 0; k < l.w*l.h*l.n; ++k){
			...
			if (iou > best_iou){
                ...
                // printf("best ios %f", best_iou);
                // printf("predid %d, true_id %d\n", pred_id, true_id);

                //change to 1 for evaluating yolo model
                if(0) {
                if(pred_id == 0) {
                    pred_id++;
       ...
       //  printf("test %d\n", isvalueinarray(1, seen_truth, count+1));
        /////////////////////
 
        for (j = 0; j < num_labels; ++j) {
            ++total;
            total_all_classes[truth[j].id] += 1;
        } 
	...
	precision = 100.*correct/(float)proposals;
        ...
        fprintf(stderr, "%5d %5d %5d\tIOU: %.2f%%\tPrecision: %.2f%%\tRecall:%.2f%%\tf1_score: %.2f%%\n", i, correct, total,  avg_iou*100/correct, precision,  recall,f1_score);
